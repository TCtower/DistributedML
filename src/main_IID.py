...
def check_cosine_similarity(global_g_list, dev):
    ...
    for i in range(n_clients):
        ...
        for j in range(len(global_g_list[0])):
            tmp_flat_list.append(np.reshape(global_g_list[i][j].cpu(), grad_len[j]))
        # print(tmp_tensor)
...
def train(workers, train_data_list, test_data):
    ...
    for iteration in range(1, iterations + 1):
        ...
        if args.aggregator == "mean":
            ...
            global_g_list = aggregator_utils.param_flip_flop(args, workers, g_list, assign_list, device)
